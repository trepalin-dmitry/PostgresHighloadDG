create or replace procedure "exchangeDeals@Save.Database.SequenceOne"(data character varying, size integer)
	language plpgsql
as $$
DECLARE
    dealsIds bigint[];
    ...
    WITH changed("exchangeDealId", "personId") as (
        INSERT INTO "exchangeDealsPersonsSequenceOne" (comment, "exchangeDealId", "personId")
            ...
            FROM JSONB_TO_RECORDSET(data :: JSONB) AS D (guid UUID, "persons" JSONB)
                     LEFT JOIN "exchangeDealsSequenceOne" AS DK ON DK.guid = D.guid
                     CROSS JOIN LATERAL
                (
                SELECT P.*
...
alter procedure "exchangeDeals@Save.Database.SequenceOne"(varchar, integer) owner to postgres;
